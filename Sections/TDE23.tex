...
In this case we have that \(A(u) = u\) and the initial condition is
\[
    g(x) = \begin{cases}
        1,      & x < 0,     \\
        2 - 2x, & 0 < x < 1, \\
...
\begin{center}
    \begin{tikzpicture}
        \begin{axis}[
                xlabel={$x$}, ylabel={$t$},
                xmin=-1, xmax=2, ymin=0, ymax=2,
                axis lines=middle,
            ]

            % Plot characteristics for\ u_0(x) = 1 for x <= 0
            \addplot[black, domain=-1:1] {x}; % Characteristic with slope 1
            \addplot[black, domain=-1:0.8] {x + 0.2}; % Characteristic with slope 1
            \addplot[black, domain=-1:0.6] {x + 0.4}; % Characteristic with slope 1
            \addplot[black, domain=-1:0.4] {x + 0.6}; % Characteristic with slope 1
            \addplot[black, domain=-1:0.2] {x + 0.8}; % Characteristic with slope 1

            % Plot characteristics for u_0(x) = 2 - 2x for 0 < x < 1
            \addplot[black, domain=0:1] {(x-0)/(2-2*0)};
            \addplot[black, domain=0:1] {(x-0.2)/(2-2*0.2)};
            \addplot[black, domain=0:1] {(x-0.4)/(2-2*0.4)};
            \addplot[black, domain=0:1] {(x-0.6)/(2-2*0.6)};
            \addplot[black, domain=0:1] {(x-0.8)/(2-2*0.8)};
    ...
    \end{tikzpicture}
\end{center}
that the characteristic do not intersect at least until \(t=\frac{1}{2}\).
...
\begin{center}
    \begin{tikzpicture}
        \begin{axis}[
            ...
            % Horizontal line for t = 1/2
            \draw[dashed, color=red] (axis cs:-1, 0.5) -- (axis cs:2, 0.5);

            % Label for the value of the solution
            \node[color=blue] at (-0.5,0.25) {$1$};
            \node[color=blue] at (0.75,0.15) {$\frac{2 - 2x}{1 - 2t}$};
            \node[color=blue] at (0.5,0.35) {?};
            \node[color=blue] at (1.5,0.25) {$0$};

        \end{axis}
    \end{tikzpicture}
\end{center}
In the region \(t < x < 2t\) where the solution is not defined yet, we can define it in the following way:
\[
...
\begin{center}
    \begin{tikzpicture}
        \begin{axis}[
            ...
            % Plot characteristics for\ u_0(x) = 1 for x <= 0
            \addplot[blue, domain=-1:1.5] {x}; % Characteristic with slope 1

            % Plot characteristics for u_0(x) = 2 - 2x for 0 < x < 1
            \addplot[blue, domain=0:1] {(x-0)/(2-2*0)};
            ...
            % Label for the value of the solution
            \node at (0.5,1) {$1$};
            \node at (0.75,0.15) {$\frac{2 - 2x}{1 - 2t}$};
            \node[color=blue] at (1,0.75) {$\frac{x}{t}$};
            \node at (1.5,0.25) {$0$};

            \addplot[blue, domain=1:1.5] {x^2/2}; % Shock speed for t > 1/2
            \node[color=blue] at (1.4,0.65) {$\eta(t)$};

        \end{axis}
    \end{tikzpicture}
\end{center}
And now we could decide to study what happens when to two blue lines intersect when $t = T^*$
